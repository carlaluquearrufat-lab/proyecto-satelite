from tkinter import *
from tkinter import simpledialog # <--- IMPORTANTE: NECESARIO PARA EL POPUP DE NUMERO
...
idiomas = {
    "es": {
        ...
        "media": "Media: -- ",
        "radar": "RADAR",
        "radarmanual": "RADAR MANUAL",
        "orbita3d": "ORBITA 3D",
        "mapa2d": "MAPA 2D",
        "mediasatelite": "MEDIA TEMP. (SATÉLITE)",
        "abrircomandos": "ABRIR COMANDOS",
        "pararradar": "PARAR RADAR",
        "limitetemp": "Límite Temp (ºC):",
        "fijar": "FIJAR",
        "comando": "Escribir comando/nota:",
        "usuario": "USUARIO",
        "guardar": "GUARDAR",
        "frequencia": "FREQ ORBITA",
    ...
    "en": {
        ...
        "media": "Average: -- ",
        "radar": "RADAR",
        "radarmanual": "MANUAL RADAR",
        "orbita3d": "3D ORBIT",
        "mapa2d": "2D MAP",
        "mediasatelite": "SATELLITE AVG TEMP",
        "abrircomandos": "OPEN COMMANDS",
        "pararradar": "STOP RADAR",
        "limitetemp": "Temp Limit (ºC):",
        "fijar": "SET",
        "comando": "Write command/note:",
        "usuario": "USER",
        "guardar": "SAVE",
        "frequencia": "ORBIT FREQ",
...
# Idioma inicial
idioma_actual = "es"
orden_idiomas = ["es", "en", "ch"]

def cambiar_idioma():
    global idioma_actual
    indice = orden_idiomas.index(idioma_actual)
    idioma_actual = orden_idiomas[(indice + 1) % len(orden_idiomas)]

    # Actualizar textos
    titulo.config(text=idiomas[idioma_actual]["version"])
    idioma_label.config(text=idiomas[idioma_actual]["label"])
    idioma_boton.config(text=idiomas[idioma_actual]["boton"])
    temp_boton.config(text=idiomas[idioma_actual]["temp"])
    hum_boton.config(text=idiomas[idioma_actual]["hum"])
    stemp_boton.config(text=idiomas[idioma_actual]["stoptemp"])
    shum_boton.config(text=idiomas[idioma_actual]["stophum"])
    media_label.config(text=idiomas[idioma_actual]["media"])
    radar_boton.config(text=idiomas[idioma_actual]["radar"])
    radarmanual_boton.config(text=idiomas[idioma_actual]["radarmanual"])
    orbita3d_boton.config(text=idiomas[idioma_actual]["orbita3d"])
    mapa2d_boton.config(text=idiomas[idioma_actual]["mapa2d"])
    mediasatelite_boton.config(text=idiomas[idioma_actual]["mediasatelite"])
    abrircomandos_boton.config(text=idiomas[idioma_actual]["abrircomandos"])
    pararradar_boton.config(text=idiomas[idioma_actual]["pararradar"])
    limitetemp_label.config(text=idiomas[idioma_actual]["limitetemp"])
    fijar_boton.config(text=idiomas[idioma_actual]["fijar"])
    comando_label.config(text=idiomas[idioma_actual]["comando"])
    guardar_boton.config(text=idiomas[idioma_actual]["guardar"])
    freq_boton.config(text=idiomas[idioma_actual]["frequencia"])
...
def RADARStopClick():
    if ser: 
        ser.write(b"RS\n")  # Enviar comando a Arduino para parar radar
...
window.title("INTERFAZ VERSION 5") 
window.rowconfigure([0,1,2,3,4,5], weight=1)
window.columnconfigure([0,1,2,3,4,5,6], weight=1)

titulo = Label(window, text=idiomas[idioma_actual]["version"], font=("Times New Roman", 20, "bold"))
titulo.grid(row=0,column=2,columnspan=2,sticky=N+S+E+W)

#Botones principales
botones = {'width':12,'height':2,'font':("Arial",11,"bold"),'relief':'raised','bd':3}
temp_boton = Button(window,text=idiomas[idioma_actual]["temp"], command=TEMPClick, bg='blue',fg='white',**botones)
temp_boton.grid(row=2,column=0,sticky=N+S+E+W)
stemp_boton = Button(window,text=idiomas[idioma_actual]["stoptemp"], command=STOPTClick, bg='blue',fg='white',**botones)
stemp_boton.grid(row=5,column=0,sticky=N+S+E+W)
hum_boton = Button(window,text=idiomas[idioma_actual]["hum"], command=HUMClick, bg='red',fg='white',**botones)
hum_boton.grid(row=2,column=1,sticky=N+S+E+W)
shum_boton = Button(window,text=idiomas[idioma_actual]["stophum"], command=STOPHClick, bg='red',fg='white',**botones)
shum_boton.grid(row=5,column=1,sticky=N+S+E+W)
media_label = Label(window, text=idiomas[idioma_actual]["media"], font=("Arial",12), bg='white', relief='sunken')
media_label.grid(row=5, column=2, sticky=N+S+E+W, padx=5, pady=5)

radar_boton = Button(window,text=idiomas[idioma_actual]["radar"], command=RADARClick, bg='green',fg='white',**botones)
radar_boton.grid(row=2,column=3,sticky=N+S+E+W)
radarmanual_boton = Button(window,text=idiomas[idioma_actual]["radarmanual"], command=RADARMClick, bg='green',fg='white',**botones)
radarmanual_boton.grid(row=2,column=4,sticky=N+S+E+W)

# Botones de Orbita y Mapa juntos
orbita3d_boton = Button(window,text=idiomas[idioma_actual]["orbita3d"], command=ORBITClick, bg='orange',fg='white',**botones)
orbita3d_boton.grid(row=2,column=5,sticky=N+S+E+W)
mapa2d_boton = Button(window,text=idiomas[idioma_actual]["mapa2d"], command=MAPAClick, bg='cyan',fg='black',**botones)
mapa2d_boton.grid(row=2,column=6,sticky=N+S+E+W)

mediasatelite_boton = Button(window, text=idiomas[idioma_actual]["mediasatelite"], command=MEDIAClick, bg='purple', fg='white', **botones)
mediasatelite_boton.grid(row=3, column=0, sticky=N+S+E+W)
abrircomandos_boton = Button(window, text=idiomas[idioma_actual]["abrircomandos"], command=abrir_fichero_comandos, bg='gray', fg='white', **botones)
abrircomandos_boton.grid(row=3, column=1, sticky=N+S+E+W)
pararradar_boton = Button(window, text=idiomas[idioma_actual]["pararradar"], command=RADARStopClick, bg='gray', fg='white', **botones)
pararradar_boton.grid(row=5, column=4, sticky=N+S+E+W)

# --- BOTÓN FRECUENCIA ORBITA ---
freq_boton = Button(window, text=idiomas[idioma_actual]["frequencia"], command=FREQ_ORBITAClick, bg='#DD8800', fg='white', **botones)
freq_boton.grid(row=5, column=5, columnspan=2, sticky=N+S+E+W)
...
limitetemp_label = Label(frame_alarma, text=idiomas[idioma_actual]["limitetemp"], bg="#eeeeee", font=("Arial", 10, "bold"))
limitetemp_label.pack(side=LEFT, padx=5)
...
fijar_boton = Button(frame_alarma, text=idiomas[idioma_actual]["fijar"], command=SET_ALARMAClick, bg='red', fg='white', font=("Arial", 9, "bold"))
fijar_boton.pack(side=LEFT, padx=5)
...
comando_label = Label(frame_comando, text=idiomas[idioma_actual]["comando"],
      bg="#dddddd", font=("Arial", 10, "bold"))
comando_label.pack()
...
guardar_boton = Button(frame_comando, text=idiomas[idioma_actual]["guardar"],
...
# PANEL IDIOMA
# Usamos un Frame pequeño para agrupar etiqueta y botón
frame_idioma = Frame(window, bd=1, relief='flat', bg="#eeeeee")
frame_idioma.grid(row=0, column=4, columnspan=1, sticky=N+S+E+W, padx=3, pady=2)

idioma_label = Label(frame_idioma, text=idiomas[idioma_actual]["label"], font=("Times New Roman", 20, "bold"))
idioma_label.pack(pady=20)
idioma_boton = Button(frame_idioma, text=idiomas[idioma_actual]["boton"], command=cambiar_idioma, font=("Arial", 14))
idioma_boton.pack()
