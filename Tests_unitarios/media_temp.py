        ...
        try:
            if ser.in_waiting > 0:
                ...
                with data_lock:
                    ...
                    # ------------------------------
                    # NUEVO: Lectura de MEDIA del Arduino
                    # ------------------------------
                    if 'MEDIA:' in linea:
                        try:
                            val = float(linea.split('MEDIA:')[-1])
                            medias_arduino.append(val)
                            print("Media desde Arduino:", val)
                        except:
                            pass
...
def plot_temp():
    ...
    while grafica_temp:
        ...
        # -------------------------------
        # NUEVO: Media calculada en PYTHON
        # -------------------------------
        if modo_media.get() == "PYTHON" and len(ys) >= 10:
            media_python = sum(ys[-10:]) / 10.0
            ax_temp.plot(xs, [media_python]*len(xs), color='green', label='MEDIA 10 (PYTHON)')

        # -------------------------------
        # NUEVO: Media enviada por ARDUINO
        # -------------------------------
        if modo_media.get() == "ARDUINO" and medias_arduino:
            media_ard = medias_arduino[-1]
            ax_temp.plot(xs, [media_ard]*len(xs), color='orange', label='MEDIA 10 (ARDUINO)')
...
def TEMPClick():
    ...
    if ser:
        if modo_media.get() == "ARDUINO":
            ser.write(b"IniciarT_ARDUINO\n")
        else:
...
# ---------------- MODO MEDIA (NUEVO) ----------------
modo_media = StringVar()
modo_media.set("PYTHON")

Label(window, text="Calcular media en:", font=("Arial",12,"bold")).grid(row=1, column=0, sticky=W)
OptionMenu(window, modo_media, "PYTHON", "ARDUINO").grid(row=1, column=1, sticky=W)
