        ...
        try:
            if ser.in_waiting > 0:
                ...
                with data_lock:
                    ...
                    if ' 3:' in linea:
                        try:
                            val = float(linea.split(' 3:')[-1].split()[0])
                    ...
                    if ' 4:' in linea:
                        try:
                            val = float(linea.split(' 4:')[-1].split()[0])
                            angulos.append(val)
                        except: pass

                    # -----------------------
                    # Lectura de media enviada por Arduino
                    # -----------------------
                    if ' 5:' in linea:
                        try:
                            val = float(linea.split(' 5:')[-1].split()[0])
                            medias_arduino.append(val)
                            # Mantener solo últimas 50 medias para no crecer indefinidamente
                            if len(medias_arduino) > 50:
                                medias_arduino.pop(0)
